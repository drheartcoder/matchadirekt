...
class Manage_Newsletters extends CI_Controller {
	public function index(){
		...
		$data=array();
		$data['title'] = SITE_NAME.': '.lang('Newsletter Management');
		$data['msg'] = '';
	...
	public function add(){
		...
		$this->form_validation->set_rules('from_name', lang('From Name'), 'trim|required');
		$this->form_validation->set_rules('from_email', lang('From Email'), 'trim|required|secure|valid_email');
		$this->form_validation->set_rules('email_subject', lang('Email Subject'), 'trim|required');
		$this->form_validation->set_rules('email_interval', lang('Email interval'), 'trim|required|numeric');
		$this->form_validation->set_rules('editor1', lang('Email Content'), 'trim|required');
	...
	public function update(){
		...
		$this->form_validation->set_rules('edit_from_name', lang('From Name'), 'trim|required');
		$this->form_validation->set_rules('edit_from_email', lang('From Email'), 'trim|required|secure|valid_email');
		$this->form_validation->set_rules('edit_email_subject', lang('Email Subject'), 'trim|required');
		$this->form_validation->set_rules('edit_email_interval', lang('email interval)', 'trim|required|numeric');
		$this->form_validation->set_rules('edit_editor1', lang('Email Content)', 'trim|required');
	    $this->form_validation->set_error_delimiters('<span class="err" style="padding-left:2px;">', '</span>');
	...
	public function status($id='', $current_staus=''){
		...
		if($current_staus==''){
			echo lang('invalid current status provided.');
			exit;
		}
	...
	public function force_send_newsletter(){
		...
		$obj_row = $this->newsletter_model->get_record_by_id($id);
		$this->form_validation->set_rules('n_force_id', lang('ID'), 'trim|required');
		$this->form_validation->set_rules('email_address', lang('Eamil Address'), 'trim|required|valid_email');
