...
function delete_posted_job(ID){
	...
	Confirm("Are you sure you want to delete this job?",function(result){
	      if(result){
...
$( document ).ready(function() {
    ...
    //$($(":file")).click(function(event){
      //  var wid = $(window).width();
        //if(wid<750)
        //{
         //   alert("Sorry , you can't upload files from mobile !");
...
function apply_job(vr=1){
	...
	//submitter
	if(returnval){
		// if($('#check_agree').prop('checked', false) && vr!=0) 
		// {
		// 	// $('#submitter_af').attr('disabled', 'disabled');
		// 	// $('#check_agree').attr('checked',false);
		// 	$('#japply_before').modal('show');
		// 	$('#japply').modal('hide');
		// 	return;
		// }
		var input = document.getElementById('attached_file');
		...
		$.ajax({
				...
				data: { jid: $("#jid").val(), cv: $("#cv").val(),old_application: $("#old_application").val(), cover_letter: $("#cover_letter").val(), expected_salary: $("#expected_salary").val(), answer: $( 'input[name=answer]:checked' ).val(), skills_level: $("#skills_level").val() }
			  })
				.done(function( msg ) {
					if(msg=='done'){
						try {
...
function del_edu(id){
		...
		Confirm("Are you sure you want to delete your education?",function(result){
			if(result){
			$('#edu_'+id).fadeOut();
			$.ajax({
					type: "POST",
...
function del_applied_job(id){
		...
		Confirm("Are you sure you want to delete this applied job?",function(result){
			if(result){
			$('#aplied_'+id).fadeOut();
			$.ajax({
					type: "POST",
...
function add_js_ref(){
		...
		$.ajax({
				type: "POST",
				url: baseUrl+"jobseeker/reference/add",
				data: { name: $("#name_ref").val(), title: $("#title_ref").val(), phone: $("#phone_ref").val(), email: $("#email_ref").val()}
			  })
				.done(function( msg ) {
					if(msg=='done'){
						...
						$('#emsg_add_ref').html('<span class="label label-warning">'+msg+'</span>');
					}
		});

}

function load_edit_js_ref(id){
			$('#edit_ref_modal').modal('toggle');
			$.ajax({
					type: "POST",
					url: baseUrl+"jobseeker/reference/reference_by_id",
					data: { id: id}
				  })
					.done(function( data ) {
						obj = jQuery.parseJSON(data);
						$("#ed_name_ref").val(obj.name);
						$("#ed_title_ref").val(obj.title);
						$("#ed_phone_ref").val(obj.phone);
						$("#ed_email_ref").val(obj.email);
						$("#ed_ref_id").val(obj.ID);
			});
}

function edit_js_ref(){
		
		$.ajax({
				type: "POST",
				url: baseUrl+"jobseeker/reference/edit",
				data: { id: $("#ed_ref_id").val(), name: $("#ed_name_ref").val(), title: $("#ed_title_ref").val(), phone: $("#ed_phone_ref").val(), email: $("#ed_email_ref").val()}
			  })
				.done(function( msg ) {
					if(msg=='done'){
						$('#edit_ref_modal').modal('toggle');
						...
						$('#emsg_edit_ref').html('<span class="label label-warning">'+msg+'</span>');
					}
		});
...
function del_ref(id){
		...
		Confirm("Are you sure you want to delete your reference?",function(result){
			if(result){
				$('#edu_'+id).fadeOut();
				$.ajax({
						type: "POST",
						url: baseUrl+"jobseeker/reference/delete",
						data: { id: id}
					  })
						.done(function( msg ) {
							if(msg=='done'){
...
function del_exp(id){
		...
		Confirm("Are you sure you want to delete your experience?",function(result){
			if(result){
				$('#edu_'+id).fadeOut();
				$.ajax({
						type: "POST",
...
function del_cv(id, fl){
		...
		Confirm("Are you sure you want to delete your resume?",function(result){
			if(result){
				$('#cv_'+id).fadeOut();
				$.ajax({
						type: "POST",
