...
class Posted_jobs_model extends CI_Model {
	...
	public function delete_posted_job_by_id_emp_id($id,$emp_id){
		$this->db->where('ID', $id);
		$this->db->where('employer_ID', $emp_id);
		$return = $this->db->update('tbl_post_jobs',['deleted'=>1]);
		return $return;
	}
	...
	public function get_matching_searched_jobs($param, $per_page, $page) {
       $Q = $this->db->query("
        	SELECT pj.ID, pj.job_title, pj.job_slug, pj.employer_ID, pj.company_ID, pj.job_description,pj.pay ,pj.required_skills,pj.experience, pj.city, pj.dated, pj.last_date, pj.is_featured, pj.sts, pc.company_name, pc.company_logo, pc.company_slug
