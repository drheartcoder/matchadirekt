...
class Applied_jobs_model extends CI_Model {
	...
	public function delete_applied_job($id){
		$this->db->where('ID', $id);
		$this->db->update('tbl_seeker_applied_for_job',['deleted'=>1]);
	}
	...
	public function get_applied_job_by_id($id) {
        ...
        $this->db->where('tbl_seeker_applied_for_job.deleted',0);
        $Q = $this->db->get();
        if ($Q->num_rows() > 0) {
            $return = $Q->row();
        } else {
    ...
    public function get_applied_job_by_job_id($job_id) {
        $this->db->from('tbl_seeker_applied_for_job');
        $this->db->join('tbl_job_seekers', 'tbl_job_seekers.ID=tbl_seeker_applied_for_job.seeker_ID', 'left');
    ...
    public function get_applied_count_job_by_seeker_and_employer_id($seeker_id, $employer_id) {
        $this->db->from('tbl_seeker_applied_for_job');
        $this->db->where('tbl_seeker_applied_for_job.seeker_ID', $seeker_id);
        $this->db->where('tbl_seeker_applied_for_job.employer_ID', $employer_id);
        $this->db->where('tbl_seeker_applied_for_job.deleted',0);
        return $this->db->count_all_results();
    }
    
    public function get_applied_count_job_by_seeker_and_employer_id_and_job_id($seeker_id, $employer_id, $job_id) {
        $this->db->from('tbl_seeker_applied_for_job');
        $this->db->where('tbl_seeker_applied_for_job.seeker_ID', $seeker_id);
        $this->db->where('tbl_seeker_applied_for_job.employer_ID', $employer_id);
        $this->db->where('tbl_seeker_applied_for_job.job_ID', $job_id);
	...
	public function get_applied_job_by_seeker_and_employer_id($seeker_id, $employer_id) {
        $this->db->from('tbl_seeker_applied_for_job');
        //$this->db->join('tbl_post_jobs', 'tbl_post_jobs.ID=tbl_seeker_applied_for_job.job_ID', 'left');
		$this->db->where('tbl_seeker_applied_for_job.seeker_ID', $seeker_id);
		$this->db->where('tbl_seeker_applied_for_job.employer_ID', $employer_id);
        $this->db->where('tbl_seeker_applied_for_job.deleted',0);
		$Q = $this->db->get();
