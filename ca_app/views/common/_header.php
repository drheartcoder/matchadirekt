<style>
  ...
  @media (max-width: 667px) {
    ...
    .lang_list{
        position: absolute;top:40px;right: 40px;z-index: 1598;background-color: white;
        box-shadow:0 0 5px  rgba(0,0,0,0.5);width: 90px;overflow: auto;max-height: 60px;display: none;
    }
</style>
<div class="topheader">
        ...
        <div class="col-md-12 col-lg-12 col-sm-12 col-xs-12" style="max-height: 55px;">
          <ul class="_top">
            <li class="sous_top" style="float: left"><a  href="<?php echo base_url();?>"><img src="<?php echo base_url('public/images/eye.png');?>" class="logo_img"/></a> </li>
              <li onclick="$('.lang_list').slideToggle();" class="btn sous_top a_right" title="Change Website Language" style="font-size: 20px;margin-top:10px;color:darkgreen;border-radius: 20px;" ><i class="fa fa-language"></i> <i class="fa fa-caret-down"></i>
                 <ul class="lang_list">
               <?php
                    $rows=$this->db->query("SELECT * FROM tbl_lang")->result();
                  
                    foreach($rows as $row)
                    {
                    ?>
                       <li><a href="<?=base_url()?>lang/<?=$row->Abbreviation?>"><?=$row->Abbreviation?></a></li>
                    <?php
                    }
                    ?>
                  </ul>
