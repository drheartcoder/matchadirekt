...
<div class="container detailinfo">
  ...
  <div class="dashiconwrp">
    ...
    <div class="row" style="overflow: hidden!important;">
        ...
        if($nothing==1)
        {
          ?><h3><b><br/>&nbsp;<i class="fa fa-close"></i>&nbsp;Nothing to Show<br/><br/></b></h3><?php
            goto end;
        }
        ?>
        ...
        foreach ($convs as $cnv):
          ...
          <li class="clearfix" style="height: 65px;">
              <a href="<?=base_url().'employer/chat/'.$cnv->id_conversation?>" >
                <img style="width: 56px;border-radius: 50%;" class="thumbnail" src="<?php $candidate_logo = ($cnv->company_logo)?$cnv->company_logo:'no_pic.jpg'; echo base_url('public/uploads/employer/'.$candidate_logo);?>" alt="avatar" />
              </a>
                <div class="about">
              <a href="<?=base_url().'jobseeker/chat/'.$cnv->id_conversation?>" >
              ...
              if($cnv->last_login_date!=""):
                ?>
                  <div class="status">
                    <small>Last Login : <?=date_formats($cnv->last_login_date, 'M d, Y')?></small>
                  </div>
                <?php
              endif;
              ?>
    ...
    <div class="col-md-9 chat">
      <div class="chat-header clearfix">
        <img style="width: 56px;border-radius: 50%;" src="<?php $candidate_logo = ($employer->company_logo)?$employer->company_logo:'no_pic.jpg'; echo base_url('public/uploads/employer/'.$candidate_logo);?>" alt="avatar" />
        
        <div class="chat-about">
          <div class="chat-with">Chat with <a href="<?=base_url().'company/'.$employer->company_slug?>"><?=$employer->first_name?></a></div>
          <div class="chat-num-messages">already <?=count($messages)?> message(s)</div>
        </div>
      </div> <!-- end chat-header -->
              ...
              else
              {
                ...
                <li>
                  <div class="message-data">
                    <span class="message-data-name"><i class="fa fa-circle online"></i> <?=$jobseeker->first_name?></span>
                    <span class="message-data-time"><?=date_formats($msg->sent_at, 'H:i - M d, Y')?></span>
                  </div>
      ...
      <form method="post" action="<?=base_url().'jobseeker/chat/send_message/'.$this->custom_encryption->encrypt_data($employer->ID)?>">
      <div class="chat-message clearfix">
        <textarea name="message" required="required" id="message-to-send" placeholder ="Type your message" rows="3"></textarea>
