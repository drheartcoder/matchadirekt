...
class MY_Form_validation extends CI_Form_validation {
    ...
    public function edit_unique($value, $params)
    {
        ...
        $result = $this->CI->db->where($field, $value)->where("ID !=", $current_id)->get($table)->row();  
        if($result)
            return false;
        return true;
    }
	public function validate_ml_spam($captcha) {
        $CI = & get_instance();
        $CI->form_validation->set_message('validate_ml_spam', 'Ogiltig verifieringskod inmatad.');
    ...
    public function time_diff($str) {
        $CI = & get_instance();
        $CI->form_validation->set_message('time_diff', 'Kan inte skicka mail för ofta.');
        $spam_validation = true;
