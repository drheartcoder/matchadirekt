...
class Messenger extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('application/seeker/inbox_view',$this->outputData);
	}
	...
	public function send_message($id = "")
	{
		...
		$conv=$conv['0'];
		$message_to_send=lang("Hi").' '.$employer_->first_name.",<br/><br/>".lang("You have new message(s) from")." <a href='".APPURL.'/seeker/messenger/chat-history/'.$conv->id_conversation."'>".$row->first_name.' '.$row->last_name."</a> :<br/><br/>";
		...
		if (!empty($_FILES['upload_file']['name']))
		{
			...
			if (!$this->upload->do_upload('upload_file')){
				...
				redirect(APPURL."/seeker/messenger/chat-history/".$conv->id_conversation);
				exit;
			}
