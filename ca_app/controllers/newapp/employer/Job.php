...
class Job extends CI_Controller {
	...
	public function index(){
		...
		//myPrint($jobList);die;
		$this->load->view('application/employer/emp_my_jobs_view',$this->outputData);
	}
		...
		if(isset($_POST['btnSubmit']) || isset($_POST['btnDraft']) ){
			//myPrint($_POST);exit;
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$this->load->view('application/employer/emp_add_new_job_view',$this->outputData);
					return;
	...
	public function edit($jobId =0,$redirectTo = 0){
		...
		if(isset($_POST['btnSubmit'])){
			...
			$this->form_validation->set_error_delimiters('<div class="errowbox"><div class="erormsg">','</div></div>');
			if ($this->form_validation->run() === FALSE) {
				$this->load->view('application/employer/edit_job_view',$this->outputData);
				return;
			}
			$quizz_text="";
			foreach ($this->input->post('quizzes') as $quizz_id)
			...
			if($redirectTo == 1)
				redirect(APPURL."/employer/company");
			else
				redirect(APPURL."/employer/job");
		}
		//myPrint($job_array);exit;
	...
	public function view_job($jobId=0,$redirectTo = 0,$archiveId = 0, $candidateId = 0){
		...
		$this->load->view('application/employer/emp_view_job',$this->outputData);
