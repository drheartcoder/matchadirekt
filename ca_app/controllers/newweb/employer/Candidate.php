...
class Candidate extends CI_Controller {
	...
	public function invite(){
		...
		if($insert){
			$compName = $this->My_model->getSingleRowData('tbl_companies',"","ID = ".$this->sessCompanyId);
			$candidateDetails = $this->My_model->getSingleRowData('tbl_job_seekers',"","ID = ".$seekerID);
			$mail_message=$candidateDetails->first_name."<br/><br/>"."The employer of this job has event for you for '".$compName->company_name."'.<br/><br/>Regards,";
			$mail = new PHPMailer();
			$mail->IsSMTP();
			$mail->SMTPAuth = true;
			...
			$mail->AddAddress($candidateDetails->email,$candidateDetails->first_name);
			$mail->Subject ="About New Invitation";
	...
	public function candidate_full_post($id=0, $redirectTo = 0,$jobId = 0,$employer_id = 0, $jobseeker_id = 0,$notType = 0){
		...
		$data['latest_job_title']		= ($row_latest_exp)?$row_latest_exp->job_title:'Fresher';
		$data['latest_job_company_name']= ($row_latest_exp)?$row_latest_exp->company_name:'';
		//$data['photo'] 					= $photo;
		$data['employer_jobs']=$this->db->query("SELECT * FROM tbl_post_jobs WHERE tbl_post_jobs.employer_ID='$this->sessUserId' AND deleted='0'")->result();
		$data['applications']=[];
		...
		if($redirectTo == 6 && $notType = 1){
			//update 
			$this->db->query("UPDATE `tbl_employer_notification` SET `isNew`=0 WHERE `seekerId`=".$decrypted_id." AND `employerId`=".$this->sessUserId." AND notificationFor = 'New Seeker'");
		}
		if($redirectTo == 6 && $notType = 2){
			//update 
			$this->db->query("UPDATE `tbl_employer_notification` SET `isNew`=0 WHERE `seekerId`=".$decrypted_id." AND `employerId`=".$this->sessUserId." AND notificationFor = 'Withdraw From Job'");
		}
		...
		$this->outputData['latest_job_title'] = $data['latest_job_title'];
		...
		$this->load->view('newweb/employer/candidate_full_post_view',$this->outputData);
	}
	...
	public function wishlisted_candidates(){
		...
		$this->load->view('newweb/employer/saved_candidate_view',$this->outputData);
	}
	...
	public function delete_wishlisted_job($id=0, $redirectTo = 0){
		...
		if($delete){
			redirect(WEBURL.'/employer/candidate/wishlisted-candidates');
		}
	}
	...
	public function view_candidate_details($id=0 ,$jobId = 0, $returnTo = 0,$applicationId = 0){
		...
		$applied_job_title = $this->My_model->getSingleRowData('tbl_seeker_applied_for_job',"","seeker_ID = ".$id." AND job_ID = ".$jobId);	
		/*myPrint($jobData);
	  ...
	  /* myPrint($applied_job_title);
		...
		$this->load->view('newweb/employer/manage_candidate_interview_process_view',$this->outputData);
	}
	...
	public function add_event($seekerId=0,$jobId=0,$applicationId = 0){
		...
		//redirect(WEBURL."/employer/candidate/view-candidate-details/".$seekerId."/".$jobId);
		$compName = $this->My_model->getSingleRowData('tbl_companies',"","ID = ".$this->sessCompanyId);
		$candidateDetails = $this->My_model->getSingleRowData('tbl_job_seekers',"","ID = ".$seekerId);
		$mail_message=$candidateDetails->first_name."<br/><br/>"."The employer of this job has added event for you for '".$compName->company_name."' on ".$date.".<br/><br/>Regards,";
		
		//notification
		/*$insertData =array();
		$insertData['seekerId'] =$seekerId;
		$insertData['employerId'] = $this->sessUserId;
		$insertData['notificationFor'] = "Add Event";
		$insertData['notificationText'] = "New event added by employer";
		...
		$mail->Port = 25;
		$mail->Send();
		redirect(WEBURL."/employer/calendar");
	}
	...
	public function update_event($seekerId=0,$jobId=0){
		...
		$compName = $this->My_model->getSingleRowData('tbl_companies',"","ID = ".$this->sessCompanyId);
		$candidateDetails = $this->My_model->getSingleRowData('tbl_job_seekers',"","ID = ".$seekerId);
		$mail_message=$candidateDetails->first_name."<br/><br/>"."The employer of this job has been change status of your application on '".$compName->company_name."' for ".$date.".<br/><br/>Regards,";

		/*//notification
		$insertData =array();
		$insertData['seekerId'] =$seekerId;
		$insertData['employerId'] = $this->sessUserId;
		$insertData['notificationFor'] = "Update Event";
		$insertData['notificationText'] = "Event updated by employer";
		$insertId  = $this->My_model->insert("tbl_seeker_notification",$insertData);*/
		...
		$mail->FromName = "BiXma Job System";
		$mail->AddAddress($candidateDetails->email,$candidateDetails->first_name);
		$mail->Subject ="About change in schedule";
		$mail->Body = $mail_message;
		$mail->WordWrap = 50;
		$mail->IsHTML(true);
		/*$mail->SMTPSecure = 'tls';
	...
	public function change_status($seeker_ID = 0,$job_id = 0 ){
		//echo $seeker_ID;	
		/*echo $job_id;	exit;*/
		$update = $this->db->query("UPDATE tbl_seeker_applied_for_job SET flag='".$this->input->post('status')."' WHERE job_ID='".$job_id."' AND tbl_seeker_applied_for_job.seeker_ID = ".$seeker_ID);
		...
		if($update)
			{ 
				...
				$mail->From ="bixma@agoujil.com";
				$mail->FromName = "BiXma Job System";
				$mail->AddAddress($data_array->jobseeker_email,$data_array->jobseeker_name);
				$mail->Subject =lang("About")." ".$data_array->job_title;
				$mail->Body = $mail_message;
				$mail->WordWrap = 50;
				$mail->IsHTML(true);
				/*$mail->SMTPSecure = 'tls';
	...
	public function callinfo(){
		...
		$seekerID = $_POST['seeker_ID'];
		$seekerData = $this->My_model->getSingleRowData('tbl_job_seekers',"","ID = ".$seekerID);
		//print_r($seekerData);die;
		echo $seekerData->mobile;
