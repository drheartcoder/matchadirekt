...
class Notification extends CI_Controller {
	...
	public function index(){
		...
		$otherNotifications = $this->My_model->SelTableData("tbl_seeker_notification","employerId employer,seekerId seeker, dated , notificationFor , notificationText , (select first_name from tbl_employers where ID = employer) employerName, jobId,  (select company_ID from tbl_employers where ID = employer) companyId, (select company_name from tbl_companies where ID = companyId) companyName ","seekerId = ".$this->sessUserId." AND isNew = 1");
		...
		if(isset($otherNotifications ) && $otherNotifications  != ""){
			foreach($otherNotifications  as $ev){
				$ev->notType= $ev->notificationFor;
			}
			$data = array_merge($data,$otherNotifications);
		}
		//myPrint($data);exit;
		$this->outputData['data'] = $data;
		$this->load->view('newweb/seeker/notification_list_view',$this->outputData);
	}
	...
	public function delete_notifications(){
		...
		$this->db->query("UPDATE `tbl_seeker_notification` SET `isNew`=0 WHERE  seekerId = ".$this->sessUserId);
		redirect(WEBURL."/seeker/notification");
	}
