...
class Scam_Report extends CI_Controller {
	...
	public function index()
	{
		...
		$this->form_validation->set_rules('reason', lang('Reason'), 'trim|required|strip_all_tags');
		$this->form_validation->set_rules('captcha', 'Captcha', 'trim|required|strip_all_tags|validate_ml_spam');
		$this->form_validation->set_rules('scjid', lang('ID'), 'trim|required|strip_all_tags');
		...
		if($this->session->userdata('is_job_seeker')!=TRUE){
			$data['msg'] = lang('You are not logged in with a jobseeker account. Please re-login with a jobseeker account submit this form.');
			$data['cap'] = create_ml_captcha();
			echo json_encode($data);
			exit;
		}
