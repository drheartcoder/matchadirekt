...
class Apply_Job extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->input->post('old_application')!="")
		{
			$res_old=$this->db->query("SELECT * FROM tbl_seeker_applied_for_job WHERE ID='".$this->input->post('old_application')."'")->result();
			if(count($res_old)>0)
			{
				$res_old=$res_old['0'];
				$cover_letter=$res_old->cover_letter;
				$expected_salary=$res_old->expected_salary;
				$attached_file=$res_old->file_name;
			}
		}
		$job_array = array(
		...
		if($this->input->get('yep',TRUE))
		{
			...
			redirect(base_url().'jobs/'.$row->job_slug);
		}
		echo 'done';
	}
}
