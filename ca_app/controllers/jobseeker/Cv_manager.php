...
class Cv_Manager extends CI_Controller {
	...
	public function cv_public($id_resume)
	{
	...
	public function cv_private($id_resume)
	{
		$id=$this->session->userdata('user_id');
		$this->db->query("UPDATE tbl_seeker_resumes SET is_default_resume='no' WHERE ID='".$id_resume."' AND seeker_ID='".$id."'");
		redirect(base_url().'jobseeker/cv_manager');
	}
}
