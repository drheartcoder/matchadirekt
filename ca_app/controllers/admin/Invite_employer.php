...
class Invite_Employer extends CI_Controller {
	public function index(){
		...
		$this->form_validation->set_rules('employer_name', lang('Employer Name'), 'trim|required');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->email->subject(SITE_NAME.' | '.lang('Invitation Request'));
			$this->email->message($mail_message);     
			$this->email->send() or die(lang('Error while sending email.'));
