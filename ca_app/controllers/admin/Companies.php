...
class Companies extends CI_Controller {
	...
	public function update($emp_id, $comp_id=''){
		...
		$data['title'] = SITE_NAME.': '.lang('Edit Employer Details');
		$data['msg'] = '';
			
		$this->form_validation->set_rules('company_name', lang('Company Name'), 'trim|required');
		$this->form_validation->set_rules('industry_ID', lang('Industry ID'), 'trim|required');
		$this->form_validation->set_rules('company_phone', lang('Company Phone'), 'trim|required');
		$this->form_validation->set_rules('company_location', lang('Company location'), 'trim|required');
		$this->form_validation->set_rules('company_website', lang('Company website URL'), 'trim|required|valid_url');
		
		$this->form_validation->set_error_delimiters('<span class="err" style="padding-left:2px;">', '</span>');
		$this->form_validation->set_message(lang('required'), '%s '.lang('field is required').'.');
