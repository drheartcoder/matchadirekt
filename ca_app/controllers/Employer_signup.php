...
class Employer_signup extends CI_Controller {
	...
	public function index()
	{
		...
		$data['title'] = lang('Create New Employer Account at ');
		$data['msg']='';
		$data['result_cities'] = $this->cities_model->get_all_cities();
        $data['channel_list']=$this->db->query("SELECT * FROM tbl_channels")->result();
		...
		$this->form_validation->set_rules('email', lang('Email'), 'trim|required|valid_email|is_unique[tbl_employers.email]|strip_all_tags');	
		$this->form_validation->set_rules('pass', lang('Password'), 'trim|required|min_length[6]|strip_all_tags');
		$this->form_validation->set_rules('confirm_pass', lang('Confirm password'), 'trim|required|matches[pass]|strip_all_tags');
		/*$this->form_validation->set_rules('full_name', lang('Your name'), 'trim|required|strip_all_tags');
		...
		$this->form_validation->set_rules('mobile_phone', lang('Mobile'), 'trim|required|strip_all_tags');
		$this->form_validation->set_rules('company_name', lang('Company name'), 'trim|required|strip_all_tags');
		$this->form_validation->set_rules('industry_id', lang('Industry'), 'trim|required|strip_all_tags');
		// $this->form_validation->set_rules('company_location', lang('Company address'), 'trim|required|strip_all_tags');
		$this->form_validation->set_rules('company_description', lang('Company Description'), 'trim|required|strip_all_tags|secure');
		$this->form_validation->set_rules('company_phone', lang('Company Phone'), 'trim|required|strip_all_tags');
		$this->form_validation->set_rules('no_of_employees', lang('No of Employees'), 'trim|required|strip_all_tags');
		$this->form_validation->set_rules('company_website', lang('Company Website'), 'trim|required|strip_all_tags');
