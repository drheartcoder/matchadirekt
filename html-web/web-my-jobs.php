<?php include 'header.php'; ?>
<div class="web-sidebar bg-white vheight-100">
<?php include 'web-sidebar.php'; ?>
</div>

   <section class="main-container vheight-100 justify-content-between">
    <div class="container">
            <div class="col-11 col-lg-9 col-xl-9 mx-auto bg-white">
        <div class="row top-header bg-l-grey align-items-center">
            <div class="col-2">
                <a href="web-settings.php" class="d-block">
                    <img src="images/back-arrow.png" class="back-arrow">
                </a>
            </div>
            <div class="col-8 text-center">
                <h2 class="mb-0">My Jobs</h2>
            </div>
        </div>
        <div class="row">
            <div class="col-12 p-0 px-2">
                <div class="card border-0 details-block">
                    <div class="card-body px-0 py-2">
                        <ul class="messages-list mb-0">
                            <li class="py-3 align-items-center px-3 bdr-btm">
                                <div class="edit-block">
                                    <ul>
                                        <li><a href="web-archive.php" class=""><img src="images/archive-white2.svg" class="w-100"></a></li>
                                        <li><a href="#" class=""><img src="images/close-white2.svg" class="w-100"></a></li>
                                        <li><a href="web-edit-job.php" class=""><img src="images/edit-pen2.svg" class="w-100"></a></li>
                                    </ul>
                                </div>
                                <a class="media" href="web-view-job.php">
                                    <div class="media-body">
                                        <div class="row align-items-center">
                                            <div class="col-6 card-title mb-0">Web Designer
                                                <p class="last-msg mb-0">Company Name Inc</p>
                                                <p class="text-d-grey mb-0">Sep 12, 2019</p>
                                            </div>
                                        </div>
                                    </div>
                                </a>
                            </li>
                            <li class="py-3 align-items-center px-3 bdr-btm">
                                <div class="edit-block">
                                    <ul>
                                        <li><a href="web-archive.php" class=""><img src="images/archive-white2.svg" class="w-100"></a></li>
                                        <li><a href="#" class=""><img src="images/close-white2.svg" class="w-100"></a></li>
                                        <li><a href="web-edit-job.php" class=""><img src="images/edit-pen2.svg" class="w-100"></a></li>
                                    </ul>
                                </div>
                                <a class="media" href="web-view-job.php">
                                    <div class="media-body">
                                        <div class="row align-items-center">
                                            <div class="col-6 card-title mb-0">Web Designer
                                                <p class="last-msg mb-0">Company Name Inc</p>
                                                <p class="text-d-grey mb-0">Sep 12, 2019</p>
                                            </div>
                                        </div>
                                    </div>
                                </a>
                            </li>
                            <li class="py-3 align-items-center px-3">
                                <div class="edit-block">
                                    <ul>
                                        <li><a href="web-archive.php" class=""><img src="images/archive-white2.svg" class="w-100"></a></li>
                                        <li><a href="#" class=""><img src="images/close-white2.svg" class="w-100"></a></li>
                                        <li><a href="web-edit-job.php" class=""><img src="images/edit-pen2.svg" class="w-100"></a></li>
                                    </ul>
                                </div>
                                <a class="media" href="web-view-job.php">
                                    <div class="media-body">
                                        <div class="row align-items-center">
                                            <div class="col-6 card-title mb-0">Web Designer
                                                <p class="last-msg mb-0">Company Name Inc</p>
                                                <p class="text-d-grey mb-0">Sep 12, 2019</p>
                                            </div>
                                        </div>
                                    </div>
                                </a>
                            </li>
                        </ul>
                        <!-- nav ul list -->
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- container -->
  </div>
 
        
</section>
   
     


<!-- section -->
<?php include 'footer.php'; ?>
